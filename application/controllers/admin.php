...
public function read()
      {
	       ...
	       move_uploaded_file($_FILES['detail']['tmp_name'], 'uploads/'.$_FILES['detail']['name']);
          //load library excel
          ...
          //Load excel file
          $objPHPExcel=$objReader->load('./uploads/'.$_FILES['detail']['name']); 
          ...
          for($i=1;$i<=1000;$i++)
          {
			  ...
			  $bday= $objWorksheet->getCellByColumnAndRow(1,$i)->getValue();
			  $sex= $objWorksheet->getCellByColumnAndRow(2,$i)->getValue();
			  $religion= $objWorksheet->getCellByColumnAndRow(3,$i)->getValue();
			  $bgroup= $objWorksheet->getCellByColumnAndRow(4,$i)->getValue();
			  $addr= $objWorksheet->getCellByColumnAndRow(5,$i)->getValue();
			  $phone= $objWorksheet->getCellByColumnAndRow(6,$i)->getValue();
			  $email= $objWorksheet->getCellByColumnAndRow(7,$i)->getValue();
			  $pass= $objWorksheet->getCellByColumnAndRow(8,$i)->getValue();
			  $fname= $objWorksheet->getCellByColumnAndRow(9,$i)->getValue();
			  $mname= $objWorksheet->getCellByColumnAndRow(10,$i)->getValue();
			  $roll= $objWorksheet->getCellByColumnAndRow(11,$i)->getValue();
              $transport= $objWorksheet->getCellByColumnAndRow(12,$i)->getValue();
			  $dormitory= $objWorksheet->getCellByColumnAndRow(13,$i)->getValue();
			  $dormitoryroom= $objWorksheet->getCellByColumnAndRow(14,$i)->getValue();
			  
              if(isset($name)){
              $data_user=array('name'=>$name, 'birthday'=>$bday,'sex'=>$sex,'religion'=>$religion,'blood_group'=>$bgroup,'address'=>$addr,'phone'=>$phone, 'email'=>$email,'password'=>$pass,'father_name'=>$fname,'mother_name'=>$mname,'class_id'=>$class_id,'roll'=>$roll,'transport_id'=>$transport,'dormitory_id'=>$dormitory,'dormitory_room_number'=>$dormitoryroom,);

              $this->user_model->add_data($data_user);
			  }
			  
          }
          redirect(base_url() . 'index.php?admin/student/'.$class_id, 'refresh');
      }
